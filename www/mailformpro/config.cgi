...
## フォームの宛先
push @mailto,'arakawa@soshinsha.com';
...
## 自動返信メールの差出人名
$config{'fromname'} = '株式会社●●●　メールフォームからのお問い合わせ';
...
## 設置者に届くメールの件名
$config{'subject'} = '[ %s ] 株式会社●●●　メールフォームからのお問い合わせ';

## 設置者に届くメールの本文整形
$_TEXT{'posted'} = <<'__posted_body__';
<_mfp_date_>
株式会社●●●　お問い合せフォームより以下のメールを受付ました。
