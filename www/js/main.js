...
//----------スタッフ紹介ページの所属店舗によって表示内容を変えるコード----------

	//スタッフの所属切り替えボタン #buttonStaffCategory0～のIDタグ要素を取得
	const buttonStaffCategory = [];
	for (let i = 0; i < 6; i++) {
	       	let buttonStaffCategoryName = "#buttonStaffCategory0"+i;
		buttonStaffCategory[i] = document.querySelector(buttonStaffCategoryName);
	}

	//スタッフ情報表示場所　#introductionStaffBoxのIDタグ要素を取得
	const introductionStaffBox = document.querySelector('#introductionStaffBox');
	//スタッフタイトル　#introductionStaffTitleのIDタグ要素を取得
	const introductionStaffTitle = document.querySelector('#introductionStaffTitle');
	//スタッフナビ　#introductionStaffNaviのIDタグ要素を取得
	const introductionStaffNavi = document.querySelector('#introductionStaffNavi');
	
	//#introductionStaffBoxのIDタグ要素がある時だけonly_StaffCategoryが発火
	...
	function only_StaffCategory(){
		...
		categoryClasses.forEach(category => {
			if (introductionStaffBox.classList.contains(category)) {
				...
				introductionStaffNavi.classList.remove(category);
				buttonStaffCategory[0].classList.remove(category);
			}
		});
		...
		//クリックされた要素のIDによってカテゴリ名を付ける
		const index = parseInt(buttonStaffCategoryId.substr(-2), 10);
		if (!isNaN(index) && index >= 0 && index < categoryClasses.length) {
		  const targetClass = categoryClasses[index];
		  introductionStaffBox.classList.add(targetClass);
		  introductionStaffNavi.classList.add(targetClass);
		//introductionStaffTitle.classList.add(targetClass);
		  buttonStaffCategory[0].classList.add(targetClass);
		}


	};

//----------スタッフ紹介ページの所属店舗によって表示内容を変えるコード　ここまで----------
